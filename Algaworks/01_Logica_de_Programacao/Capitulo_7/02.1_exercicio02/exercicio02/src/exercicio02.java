...
public class exercicio02 {
    public static void main(String[] args) {
        ...
        double valorTotal;
        double maiorFaturalmento = 0;
        int semana = 0;

        for (int i = 0; i < mes.length; i++) {
            valorTotal = 0;
            Double[] totalSemana = mes[i];
            for (int j = 0; j < totalSemana.length; j++) {
                valorTotal += totalSemana[j];
                if (valorTotal > maiorFaturalmento) {
                    maiorFaturalmento = valorTotal;
                    semana = (i + 1);
                }
            }
        }

        System.out.println("A semana " + semana + " obteve o maior faturamento de: R$" + maiorFaturalmento);
